...
7. 결과 요약 → out/training_results.json
...
def main():
    ...
    section("4. 풍상측 노출 + 직전1년 민원율")
    lab = spatial_features.run(lab)
    lab = spatial_features.add_prior_rate(lab)

    section("5. 시차·무풍 피처")
    feat = build_features.run(lab)
    RESULTS["features"] = {"rows": len(feat),
                        "n_features": len(build_features.FULL_FEATURES),
                        "features": build_features.FULL_FEATURES}

    section("6. 그룹별 학습 + grouped CV")
    RESULTS["train"] = train_model.run(feat)

    section("결과 저장")
    RESULTS["findings"] = FINDINGS
    with open(OUT_DIR / "training_results.json", "w", encoding="utf-8") as fh:
        json.dump(RESULTS, fh, ensure_ascii=False, indent=2, default=str)
    print(f"  저장: {OUT_DIR / 'training_results.json'}")
