...
python demo_v6r.py   (demo_v6.py 선실행 권장 — gimje_farms_ri.csv 를 쓴다)
"""
...
def main():
    ...
    from etl import s0_clean, s0b_gimje, s1_label, s2_features, s2b_spatial
    ...
    lab = s2b_spatial.run_regions(lab)

    section("v6r-3 S2b — 지역별 직전1년 민원율")
    lab = s2b_spatial.add_prior_rate_region(lab)

    section("v6r-4 S2'' — 시차·무풍 피처 (지역 경계 유지)")
    feat = s2_features.run_v6r(lab)

    section("v6r-5 S3'' — 그룹별 학습 + 동일시험지 비교 + grouped CV")
    RESULTS["s3_v6r"] = s3_train.run_v6r(feat)
