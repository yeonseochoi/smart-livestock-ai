...
def run_legacy(lab: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"  피처 테이블 {len(f):,}행 / full {len(FULL_FEATURES_LEGACY)}개, reduced {len(REDUCED_FEATURES_LEGACY)}개")
    return f


# ═══════════════════════════════════════════════════════════════════
# 현행 — 시차·무풍 + 공간 + 직전1년 민원율
# ═══════════════════════════════════════════════════════════════════

from preprocess.spatial_features import SPATIAL_FEATURES, PRIOR_FEATURES  # noqa: E402

# v5 유산 — 기존 트랙 재현용. 절대 수정하지 말 것. (= 위의 FULL_FEATURES_LEGACY)
# (v5 별칭 제거됨 — FULL_FEATURES_LEGACY 를 직접 쓴다)

# 신규 시계열 — 냄새 도달 시간이 중앙값 32분이므로
...
REDUCED_FEATURES = ["temp", "rain", "hour", "night",
...
def _base_features(f: pd.DataFrame) -> pd.DataFrame:
    f = f.copy()
...
def run(lab: pd.DataFrame) -> pd.DataFrame:
    f = add_lag_features(_base_features(lab))
    f.to_parquet(MID_DIR / "features.parquet")
    print(f"  피처 테이블 {len(f):,}행 / full {len(FULL_FEATURES)}개")
    return f


def add_lag_features_regional(f: pd.DataFrame) -> pd.DataFrame:
    """지역 격자 — 시차는 지역 경계를 넘으면 안 된다. groupby('region') 으로 계산."""
    parts = []
...
def run_regional(lab: pd.DataFrame) -> pd.DataFrame:
    f = add_lag_features_regional(_base_features(lab))
    for c in FULL_FEATURES:
        if f[c].dtype == "float64":
            f[c] = f[c].astype("float32")
    f.to_parquet(MID_DIR / "features_regional.parquet")
    print(f"  피처 테이블 {len(f):,}행 / full {len(FULL_FEATURES)}개")
    return f
...
def build_serving_features(b: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    return add_lag_features(_base_features(b))
